...
* [ ] 1. Password less ssh from Bastion and all master and worker nodes.  On bastion host as the root user do the following
```
cd /root/.ssh
ls
```
If you do not see a file called `id_rsa.pub` then do the following:

Generate public and private keys via this command:
```
ssh-keygen -q -t rsa -N "" -f /root/.ssh/id_rsa
```
this creates 2 id files (`id_rsa` & `id_rsa.pub`), see below

```
[root@ip-172-31-1-57 ~]# ls -l /root/.ssh
total 12
-rw-------. 1 root root  551 Nov 21 17:58 authorized_keys
-rw-------. 1 root root 1679 Nov 21 18:38 id_rsa
-rw-r--r--. 1 root root  428 Nov 21 18:38 id_rsa.pub
```

We need the content of the `id_rsa.pub` file, so run this command
```
cat /root/.ssh/id_rsa.pub
```
Now place contents of this in beginning of `/root/.ssh/authorized_keys` on each and master worker node. Login to each master & worker node
and become the root user
```
sudo su -
vi /root/.ssh/authorized_keys
```
now paste the contents at the type of the file via `O` command to open the editor above the current line.  Save the file.    
Verify you can ssh as the root user from the kubernetes master to each worker node via
```
ssh -l root hostname-of-worker-node
```
you should be able to log in without a password prompt.   

If you already have a `root/.ssh/id_rsa.pub` file on the bastion host then copy the content of that file to each master & worker node to the end of the `/root/.ssh/authorized_keys` file on each node. 
